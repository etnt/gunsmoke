...
continue(CfgMap) ->
    try
        ...
        Dispatch =
            cowboy_router:compile(%% {HostMatch, list({PathMatch, Handler, InitialState})}
...
start_listener(#{use_tls := true} = CfgMap, Dispatch) ->
    ...
    Opts = [{ip, IP}, {port, Port}] ++ TLS_opts,

    ?dbg("starting HTTPS listener on Port: ~p , Opts: ~p~n", [Port, Opts]),
    cowboy:start_tls(https_listener, Opts, #{env => #{dispatch => Dispatch}});
%%
